...
export class MixedContentHelper {
  ...
  hasMixedContentIssue(): boolean {
    ...
    const apiUrl = "http://13.61.176.255/anansiai"; // Current API is HTTP only
  ...
  getAvailableSolutions(): MixedContentSolution[] {
    const solutions: MixedContentSolution[] = [
      {
        type: "https",
        url: "https://13.61.176.255/anansiai",
        description:
  ...
  async testHttpsApi(): Promise<{ success: boolean; error?: string }> {
    try {
      const httpsUrl = "https://13.61.176.255/anansiai/api/Institutions";
  ...
  async createFallbackClient() {
    ...
    const baseURL = httpsTest.success
      ? "https://13.61.176.255/anansiai"
      : "http://13.61.176.255/anansiai"; // This will fail in HTTPS context but provides clear error
...
Quick Solutions:
1. 🔧 Configure SSL certificate on your API server (13.61.176.255)
...
• API URL: http://13.61.176.255/anansiai (HTTP)
